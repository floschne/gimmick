...
class OAIJudge(LMMJudge):
    def __init__(
        ...
        if model_id not in supported_models:
            raise ValueError(f"hf_model_id must be one of , got {model_id}")
        ...
        logger.info(f"Loading LlavaCritic {model_id}")
        self.tokenizer, self.model, self.image_processor, self.max_length = (
            load_pretrained_model(
                model_id, None, self.model_type, device_map=self.device_map
            )
        )
        self.model.eval()
